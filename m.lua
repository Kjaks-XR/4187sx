...
local esp = {
    -- settings
    enabled = true, -- ESP varsayılan olarak açık
    teamcheck = false, -- Takım kontrolü kapalı, herkes için ESP
    visiblecheck = false,
    outlines = true,
    limitdistance = false,
    shortnames = false,
    ...
    enemy_boxes = { true, Color3.new(1, 0, 0), Color3.new(0, 0, 0), 0.5 }, -- Kırmızı kutular
    enemy_healthbar = { true, Color3.new(1, 0, 0), Color3.new(0, 1, 0) }, -- Sağlık barı
    enemy_kevlarbar = { false, Color3.new(), Color3.new() },
    enemy_arrow = { false, Color3.new(), 0.5 },
    enemy_names = { true, Color3.new(1, 1, 1) }, -- Beyaz isimler
    enemy_weapon = { true, Color3.new(1, 1, 1) }, -- Beyaz silah ismi
...
function esp:update()
    ...
    for plr, drawing in next, esp.players do
        ...
        if not player then
            esp:remove(player)
            print("Oyuncu kaldırıldı:", plr)
            continue
        end
        if not esp.enabled then
            esp:hideall(drawing)
            print("ESP kapalı, gizleniyor:", plr)
            continue
        end
        if not esp.checkalive(player) then
            esp:hideall(drawing)
            print("Oyuncu canlı değil, gizleniyor:", plr)
            continue
        end
        local character = esp.getcharacter(player)
        if not character or not character.PrimaryPart then
            esp:hideall(drawing)
            print("Karakter veya PrimaryPart eksik, gizleniyor:", plr)
            continue
        end
        ...
        if not pass then
            esp:hideall(drawing)
            print("Oyuncu kontrolleri geçemedi:", plr)
            continue
        end
        ...
        if drawing.arrow.Visible then
            ...
            if esp.arrowinfo then
                ...
                if drawing.arrow_name.Visible then
                    ...
                    drawing.arrow_name.Font = 2 -- Özel font yerine varsayılan
        ...
        if esp[flag .. 'weapon'][1] then
            ...
            for _, obj in pairs(character:GetDescendants()) do
                if obj.Name:lower() == "itemroot" then
                    weaponName = obj.Parent and obj.Parent.Name:lower() or "none"
                    break
                end
            end
            drawing.weapon.Text = weaponName
            drawing.weapon.Font = 2
